...
import { Bot, Send, Sparkles, ArrowRight, ClipboardList, Zap } from 'lucide-react';
...
export default function AiDiagnosticPage() {
  ...
  useEffect(() => {
    try {
      const session = JSON.parse(localStorage.getItem('buscador-session') || '{}');
      if (session?.state?.userType) {
        setUserType(session.state.userType);
      }
    } catch (e) {}
  }, []);
  ...
  const handleAnalise = async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/ai/diagnostic`, {
        ...
        body: JSON.stringify({ description, isTechnician: isTech })
      });
  ...
  return (
    <main className="mx-auto max-w-2xl px-4 py-10 sm:px-6">
      <header className="text-center mb-12 space-y-4">
        ...
        <h1 className="text-3xl font-bold">{isTech ? 'Co-Piloto Técnico Especializado' : 'Assistente de IA TechFix'}</h1>
        <p className="text-slate-400">
          {isTech 
            ? 'Descreva o sintoma do equipamento e nossa IA ajudará com um diagnóstico avançado e testes sugeridos.'
            : 'Descreva o problema e nossa IA dirá o que pode ser antes de você contratar o técnico.'}
        </p>
      </header>
      ...
      <div className="space-y-6">
        <div className="glass-card p-6 space-y-4">
           ...
           <textarea 
              ...
              placeholder={isTech 
                ? "Ex: Ar condicionado LG Inverter dando erro CH-21, compressor não arma..." 
                : "Ex: Minha geladeira parou de gelar na parte de baixo mas o freezer continua funcionando..."}
              value={description}
           ...
           >
             {loading ? 'Analisando com Gemini Pro...' : (
               <>
                 {isTech ? 'Gerar Laudo e Dicas de Reparo' : 'Analisar Problema com IA'} <Sparkles size={18} />
               </>
             )}
           </button>
        </div>
              ...
              {isTech && (
                <div className="mt-8 pt-6 border-t border-white/10 text-center">
                   <p className="text-xs text-slate-500 italic">
                     Atenção: Este é um suporte de IA para direcionamento técnico. Use seu próprio julgamento profissional ao realizar testes físicos, e sempre utilize EPIs apropriados.
                   </p>
                </div>
              )}
            </motion.div>
